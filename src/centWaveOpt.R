...
files <- list.files(path = "pQC", recursive = TRUE, full.names = TRUE, pattern = format)
...
prefilter.scan.abundance <- seq(from = 1000, to = 100000, by = 1000)
bandwidth <- seq(from = 0.1, to = 5, by = 0.1)

## Chekc total amount of iterations
## length(peakwidth.min) + length(peakwidth.max) + length(ppm) + length(snthresh) +
## length(mzdiff) + length(prefilter.scan.number) + length(prefilter.scan.abundance) +
## length(bandwidth)
